        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    Profile profile = new Profile { UserName = model.Email, Password = model.Password };
                    travelContext db = new travelContext();
                    db.Profiles.Add(profile);
                    db.SaveChanges();
                    string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
                    var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                    await UserManager.SendEmailAsync(user.Id, "Potwierdz swoje konto", "Prosze potwierdz swoj email klikajac <a href=\"" + callbackUrl + "\">here</a>");
